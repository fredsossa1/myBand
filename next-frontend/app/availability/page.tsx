...
function BulkActionModal({ isOpen, onClose, onApply, events, state }: BulkActionModalProps) {
  const [selectedDates, setSelectedDates] = useState<string[]>([]);
  
  const stateText = state === 'A' ? 'Available' : state === 'U' ? 'Unavailable' : 'Uncertain';
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="glass border-white/20 max-w-md">
        ...
        <div className="space-y-4">
          ...
          >
            {selectedDates.length === events.length ? 'Deselect All' : 'Select All'}
          </Button>
          ...
          <div className="flex gap-2 pt-4">
            ...
            <Button 
              ...
              className={`flex-1 text-white ${
                state === 'A' ? 'bg-green-600 hover:bg-green-700' :
                state === 'U' ? 'bg-red-600 hover:bg-red-700' :
...
export default function AvailabilityPage() {
  ...
  if (events && members) {
    events.forEach(event => {
      ...
      if (currentUser && dayAvail[currentUser.id] && dayAvail[currentUser.id] !== '?') {
        stats.userResponses++;
      }
    });
  }
  
  const overallRate = stats.totalMembers && stats.totalEvents 
    ? Math.round((stats.totalResponses / (stats.totalMembers * stats.totalEvents)) * 100)
    : 0;
  const userRate = currentUser && stats.totalEvents 
      ...
      <div className="container mx-auto max-w-6xl space-y-6">
                ...
                <div className="flex gap-2">
                  {(['A', 'U', '?'] as AvailabilityState[]).map((state) => (
                    <Button
                      ...
                      className={`bg-white/10 border-white/20 text-white hover:bg-white/20 ${
                        state === 'A' ? 'hover:border-green-500/50' :
                        state === 'U' ? 'hover:border-red-500/50' :
        ...
        <Card className="glass border-white/20">
          <CardContent className="p-4">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-center">
              ...
              <div>
                <div className="text-2xl font-bold text-white">{overallRate}%</div>
                <div className="text-white/60 text-sm">Overall Response Rate</div>
              </div>
              {currentUser && (
                <div>
                  <div className="text-2xl font-bold text-white">{userRate}%</div>
                  <div className="text-white/60 text-sm">Your Response Rate</div>
                </div>
              )}
            </div>
          </CardContent>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-white">
                    ...
                    <div className="text-sm text-white/70">Press Ctrl+Z or click to undo</div>
                  </div>
                </div>
                <Button 
                  variant="outline" 
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {currentUser && (
                      <div className="space-y-3">
                        <h4 className="text-white font-medium">🎯 Your Availability</h4>
                        <div className="flex items-center gap-2">
                          <Button
                            onClick={() => {
                          ...
                          {Array.from(pendingChanges.values()).some(
                            change => change.date === event.date && change.personId === currentUser.id
                          ) && (
                                  ...
                                  return (
                                    <div 
                                      ...
                                      className={`flex items-center justify-between p-3 rounded-lg bg-white/5 border ${
                                        isCurrentUser ? 'border-blue-500/30 bg-blue-500/10' : 'border-white/10'
                                      }`}
                                    >
                                      ...
                                      <Badge 
                                        variant="outline" 
                                        className={`${
                                          state === 'A' ? 'border-green-500/50 text-green-300' :
                                          state === 'U' ? 'border-red-500/50 text-red-300' :
        ...
        <Dialog open={showKeyboardHelp} onOpenChange={setShowKeyboardHelp}>
          <DialogContent className="glass border-white/20">
            <DialogHeader>
              <DialogTitle className="text-white">⌨️ Keyboard Shortcuts</DialogTitle>
              <DialogDescription className="text-white/70">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="text-white/80"><strong>Ctrl/⌘ + R</strong> - Refresh data</div>
                <div className="text-white/80"><strong>Ctrl/⌘ + Z</strong> - Undo last action</div>
                <div className="text-white/80"><strong>Ctrl/⌘ + S</strong> - Save changes</div>
                <div className="text-white/80"><strong>?</strong> - Show this help</div>
              </div>
            </div>
          </DialogContent>
        </Dialog>
