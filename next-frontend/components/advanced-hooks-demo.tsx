...
export function AdvancedHooksDemo() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="glass border-white/20">
        ...
        <CardContent className="space-y-4">
          <div className="text-white/80">
            <p>This demo showcases the advanced React hooks that replicate vanilla JavaScript functionality:</p>
            <ul className="list-disc list-inside mt-2 space-y-1">
              <li><strong>useAvailability</strong>: Optimistic updates, pending changes, undo functionality</li>
              <li><strong>useKeyboardShortcuts</strong>: Global keyboard shortcuts (Ctrl+R, Ctrl+Z, Ctrl+S, etc.)</li>
              <li><strong>Batch operations</strong>: Submit/discard multiple changes at once</li>
              <li><strong>Real-time state</strong>: Local state merging with server data</li>
            </ul>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="glass border-white/20">
        <CardHeader>
          <CardTitle className="text-white text-lg">👤 User Selection</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="glass border-white/20">
        <CardHeader>
          <CardTitle className="text-white text-lg">📅 Availability Management</CardTitle>
        </CardHeader>
        <CardContent>
          {currentUser ? (
            <div className="space-y-4">
              {events?.slice(0, 3).map((event) => {
                const userAvailability = getUserAvailability(event.date, currentUser.id) || '?';
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        className={`
                          ...
                          ${userAvailability === 'A' ? 'bg-green-500/20 border-green-500/30' : ''}
                          ${userAvailability === 'U' ? 'bg-red-500/20 border-red-500/30' : ''}
                          ${userAvailability === '?' ? 'bg-gray-500/20 border-gray-500/30' : ''}
                        `}
                      >
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => setAvailabilityRemote(event.date, currentUser.id, cycle(userAvailability))}
      ...
      <Card className="glass border-white/20">
                  ...
                  {shortcuts.getShortcuts().map((shortcut, index) => (
                    <div key={index} className="flex items-center justify-between p-3 rounded bg-white/5">
                      <span className="text-white font-mono text-sm">
                        {shortcut.ctrlKey || shortcut.metaKey ? '⌘/Ctrl + ' : ''}
        ...
        <CardContent>
          ...
          <div className="mt-4 text-white/60 text-sm">
            <p>💡 Shortcuts are automatically disabled when typing in form fields</p>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="text-white font-mono">{shortcuts.isEnabled ? 'Yes' : 'No'}</div>
            </div>
            <div>
              <div className="text-white/60">Current User</div>
              <div className="text-white font-mono">{currentUser?.name || 'None'}</div>
            </div>
          </div>
        </CardContent>
      </Card>
