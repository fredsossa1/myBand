...
export function RoleMemberList({
  ...
  const getAvailabilityForMember = (
    memberId: string
          ...
          <Card key={role} className="glass border-white/20">
            <CardHeader className="pb-4">
              <CardTitle className="flex items-center gap-3">
                ...
                {selectedDate && (
                  <div className="text-sm text-white/80">{selectedDate}</div>
                )}
              </CardTitle>
            </CardHeader>
  ...
  return (
    ...
    >
      {getAllRoles().map((role) => {
        ...
        const responseRate =
          stats.total > 0
            ? Math.round(((stats.total - stats.noResponse) / stats.total) * 100)
            : 0;
